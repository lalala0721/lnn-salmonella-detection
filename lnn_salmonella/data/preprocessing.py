...
def read_labels(csv_path: Path) -> pd.DataFrame:
    ...
    # 将相对路径转为绝对路径
    base_dir = csv_path.parent.parent  # 负样本或 serotype_data 的根目录
...
def build_sequence_samples(
    ...
    # 只处理 DataFrame 中出现的基因组（按 split 过滤）
    ...
    for genome_id in df_genome_ids:
        ...
        # 取 num_chunks 个 chunk 作为一个序列样本
        selected = chunks_copy[:num_chunks]

        # 取第一个 chunk 的标签作为整个基因组的标签（同一基因组内标签一致）
