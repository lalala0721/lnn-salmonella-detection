...
CHUNK_SIZE = 1000
WINDOW_STRIDE = 500
RAW_CHUNKS_PER_GENOME = 150  # 每个原始基因组多采一些 chunks，增加覆盖
    ...
    for gid in list(genome_to_chunks.keys()):
        for i, c in enumerate(genome_to_chunks[gid]):
            if c[0] == 'raw_chunk':
                ...
                genome_to_chunks[gid][i] = (vec, c[2])  # 替换为 (kmer_vec, label)

    # 替换预切为 kmer_vec（只处理首元素为字符串的 tuple）
    for gid in genome_to_chunks:
        ...
        for c in genome_to_chunks[gid]:
            if isinstance(c[0], str) and c[0] == 'precut':
                ...
                pass  # 原始 chunk 已在上面编码，不应出现在这里
            else:
                new_chunks.append(c)  # already (numpy_array, label)
