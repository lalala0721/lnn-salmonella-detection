...
class CNN1DBaseline(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (batch, num_chunks, kmer_dim) -> (batch, kmer_dim, num_chunks)
        x = x.transpose(1, 2)
        x = self.conv(x)          # (batch, last_ch, reduced_len)
...
class BiLSTMBaseline(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (batch, num_chunks, kmer_dim)
        lstm_out, _ = self.lstm(x)  # (batch, num_chunks, hidden*2)

        # 注意力池化
        attn_scores = self.attn(lstm_out).squeeze(-1)  # (batch, num_chunks)
        attn_weights = F.softmax(attn_scores, dim=-1).unsqueeze(-1)  # (batch, num_chunks, 1)
...
class TransformerBaseline(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (batch, num_chunks, kmer_dim)
        x = self.input_proj(x)  # (batch, num_chunks, d_model)
        x = self.pos_encoder(x)
        x = self.encoder(x)     # (batch, num_chunks, d_model)

        # 注意力池化
        attn_scores = self.pool(x).squeeze(-1)  # (batch, num_chunks)
