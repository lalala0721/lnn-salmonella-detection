...
def extract_hidden_trajectories(model, loader, max_samples=500):
    ...
    for data, targets in loader:
        ...
        # 获取隐藏状态轨迹
        with torch.no_grad():
            traj = model.get_hidden_trajectory(data)  # (batch, chunks, hidden)
            logits = model(data)
...
def plot_tsne_trajectories(trajectories, labels, save_path='results/tsne_trajectories.png'):
    ...
    # 展平最后时间步的隐藏状态
    terminal_states = sampled_traj[:, -1, :]  # (n_plot, hidden_dim)
    ...
    # 按预测置信度着色
    probs = np.array([1 if l == 1 else 0 for l in sampled_labels])  # placeholder
    # 重绘: 正确 vs 错误
    ax2 = axes[1]
    correct_colors = []
    for i in range(len(sampled_labels)):
        # 这里简化: label 本身就是 ground truth
...
def plot_hidden_dynamics(trajectories, labels, preds, save_path='results/hidden_dynamics.png'):
    ...
    for ax_idx, (group_name, indices) in enumerate(groups.items()):
        ...
        for i in sampled:
            # 取隐藏状态的 L2 范数作为激活强度
            activation = np.linalg.norm(trajectories[i], axis=1)  # (n_chunks,)
...
def chunk_importance_gradient(model, loader, n_samples=100):
    ...
    for data, targets in loader:
        ...
        for i in range(data.size(0)):
            ...
            grad = data.grad[i].abs().mean(dim=-1)  # (n_chunks,) — 每个 chunk 的平均梯度
